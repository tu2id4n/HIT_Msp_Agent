...
def move_moving_bombs_to_next_position(prev_obs, obs):
    def is_moving_direction(bomb_pos, direction):  # 炸弹是否在移动
        ...
        if not utility.position_on_board(prev_obs['board'], rev_pos):  # 如果上一帧位置不在 board 上, 返回 False
            return False
...
def get_filtered_actions(obs, prev_two_obs=(None, None)):
    ...
    if prev_two_obs[-1] is not None:
        obs = move_moving_bombs_to_next_position(prev_two_obs[-1], obs)  # 获取下一帧炸弹移动位置
    ret = _compute_safe_actions(obs, exclude_kicking=False, prev_two_obs=prev_two_obs)
    obs = obs_cpy
    if len(ret) != 0:
        return list(ret)
    else:
